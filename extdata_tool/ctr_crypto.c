...
void ctr_keyscrambler(u8 *normalkey, u8 KeyX[16], u8 KeyY[16])
{
	...
	u8 Hash[0x20];
	ctr_sha(Source,0x20,Hash,CTR_SHA_256);
	u8 _KeyX[16];
	u8 _KeyY[16];
...
{
	switch(mode){
		case(CTR_SHA_1): sha1(data, size, hash); break;
		case(CTR_SHA_256): sha2(data, size, hash, 0); break;
	}
}
